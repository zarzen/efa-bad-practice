...
void get_worker_efa_addr(WorkerMemory* shm_w, char* addr_buf) {
  shm_w->mem_lock("lock at get_worker_efa_addr, err");
  memcpy(addr_buf, shm_w->efa_add_ptr, shm_w->efa_addr_size);
  shm_w->mem_unlock("unlock at get_worker_efa_addr, err");
};

void set_peer_addr(WorkerMemory* shm_w, char* addr_buf) {
  shm_w->mem_lock("insert remote efa addr lock err");
  ...
  *((double*)(shm_w->instr_ptr)) = timestamp;
  shm_w->mem_unlock("insert remote efa addr unlock err");
}

int get_worker_status(WorkerMemory* w) {
  w->mem_lock("lock while getting worker status");
  int s = *(int*)(w->status_ptr);
  w->mem_unlock("unlock while getting worker status");
  return s;
};

int get_worker_cntr(WorkerMemory* w) {
  w->mem_lock("getting cntr, lock err");
  int c = *(int*)w->cntr_ptr;
  w->mem_unlock("getting cntr, unlock err");
  return c;
};
