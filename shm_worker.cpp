...
class SHMWorker {
 ...
 public:
  SHMWorker(std::string name) {
    ...
    //
    shm_instr = std::string(name + "-instr-mem");
    shm_cntr = std::string(name + "-cntr-mem");
    shm_data_buf = std::string(name + "-data-buf-mem");
    shm_efa_addr = std::string(name + "-efa-addr-mem");
    shm_w_status = std::string(name + "-worker-status-mem");
  ...
  Instruction* read_instr() {
    ...
    if (ts == 0) {
      ...
      INSTR_T i_type = instr_map(*((int*)((char*)instr_ptr + 8)));
      i->type = i_type;
      i->data = ((char*)instr_ptr) + 12;

      // wipe first 12 bit to inidicate message already read
      std::fill_n(instr_ptr, 12, 0);
      return i;
    }
  };
  ...
  void set_local_efa_addr(EFAEndpoint* efa) {
    ...
    char readable[64] = {0};
    efa->get_name(local_ep_addrs, efa_addr_size);
    size_t len = efa_addr_size;
    fi_av_straddr(efa->av, local_ep_addrs, readable, &len);
    std::cout << "Local ep addresses: \n" << readable << "\n";
    std::memcpy(efa_add_ptr, local_ep_addrs, efa_addr_size);
  };
  ...
  void set_remote_efa_addr(EFAEndpoint* efa, Instruction* i) {
    ...
    char name_buf[efa_addr_size];
    char readable[64] = {0};
    size_t len = 64;
    fi_av_lookup(efa->av, efa->peer_addr, name_buf, &len);
    len = 64;
    std::fill_n(readable, efa_addr_size, 0);
  ...
  void efa_send_recv_instr(EFAEndpoint* efa, Instruction* i, bool is_send) {
    if (is_send) {
      ...
      fi_recv(efa->ep, data_buf_ptr, 64, NULL, FI_ADDR_UNSPEC, NULL);
      wait_cq(efa->rxcq, 1);
      std::cout << name << ": efa recv msg: " << data_buf_ptr << "\n";
    }
    (*(int*)cntr_ptr) += 1;
  };
  ...
  void efa_send_recv_param(EFAEndpoint* efa, Instruction* i, bool is_send) {
    ...
    if (is_send) {
      for (int i = 0; i < total_size / batch_p_size; ++i) {
        char* _buf_s = ((char*)data_buf_ptr) + i * batch_p_size;
    ...
    (*(int*)cntr_ptr) += total_size / batch_p_size;
  }
